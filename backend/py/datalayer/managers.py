...
class  CustomerManager:
    ...
    def remove(self, code):
        ...
        try:
            cursor = connection.cursor()
            cursor.execute("DELETE FROM ac_customer WHERE CODE = :code",{"code": code})
            if cursor.rowcount == 0:raise DataLayerException(f"No customer found with code {code}")
            connection.commit()
...
class InvoiceManager:
    ...
    def add(self, invoice):
        ...
        try:
            ...
            connection.commit()
            invoice.code = code_var.getvalue()
            return invoice.code
        except Exception as e:
            connection.rollback()
            raise DataLayerException(str(e))
    ...
    def getAll(self):
        ...
        """)
            ...
            for row in rows:
                invoice = {
                "code": row[0],
                "invoiceDate": row[1].strftime("%Y-%m-%d"),
                "createdOn": row[2].strftime("%Y-%m-%d") if row[2] else None,
                "customerName": row[3],
